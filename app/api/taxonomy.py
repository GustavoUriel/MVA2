...
class TaxonomyLoadDefault(Resource):
    ...
    def post(self):
        ...
        try:
            ...
            log_step("Construct default file path", "SUCCESS", 'taxonomy', 
                    file_path=default_file_path)
            
            log_step("Check file existence", "START", 'taxonomy')
            log_file_operation("CHECK_EXISTS", default_file_path, 'taxonomy')
            if not os.path.exists(default_file_path):
                log_warning("Default taxonomy file not found", 'taxonomy', 
                          file_path=default_file_path)
                log_api_response(404, '/taxonomy/load-default', 'taxonomy', 
            ...
            log_step("Process dataframe rows", "START", 'taxonomy', total_rows=len(df))
            # Create taxonomies from dataframe
            for idx, row in df.iterrows():
                try:
                    if idx % 100 == 0:  # Log progress every 100 records
                        log_step("Processing batch", "PROGRESS", 'taxonomy', 
                               row_index=idx, records_added=records_added, 
                               failed_records=failed_records)
                    
                    taxonomy_data = row.to_dict()
                    # Remove NaN values
                    original_keys = len(taxonomy_data)
                    taxonomy_data = {k: v for k, v in taxonomy_data.items() if pd.notna(v)}
                    cleaned_keys = len(taxonomy_data)
                    
                    if idx < 5:  # Log details for first 5 records
                        log_step("Process row data", "DETAIL", 'taxonomy', 
                               row_index=idx, original_keys=original_keys, 
                               cleaned_keys=cleaned_keys, sample_data=str(taxonomy_data)[:200])
                    
                    # Create taxonomy record
                    log_database_operation("INSERT", "taxonomies", 'taxonomy', 
                                         user_id=current_user.id, row_index=idx)
                    taxonomy = Taxonomy.create_from_dict(current_user.id, taxonomy_data)
                    records_added += 1
                    
                    # Commit every 100 records to avoid memory issues
                    if records_added % 100 == 0:
                        log_step("Batch commit", "START", 'taxonomy', records_committed=records_added)
                        db.session.commit()
                        log_step("Batch commit", "SUCCESS", 'taxonomy')
                        
                except Exception as e:
                    failed_records += 1
                    log_error(e, f"Failed to create taxonomy from row {idx}", 'taxonomy', 
                             row_index=idx, row_data=str(row.to_dict())[:200])
                    current_app.logger.warning(f"Failed to create taxonomy from row {idx}: {str(e)}")
                    continue
            
            log_step("Final commit", "START", 'taxonomy')
            db.session.commit()
            log_step("Final commit", "SUCCESS", 'taxonomy')
            
            log_step("Process dataframe rows", "SUCCESS", 'taxonomy', 
                    total_rows=len(df), records_added=records_added, failed_records=failed_records)
            ...
            log_api_response(200, '/taxonomy/load-default', 'taxonomy', 
                           records_added=records_added, previous_replaced=existing_count,
                           failed_records=failed_records)
            return result, 200
    ...
    def get(self):
        ...
        try:
            ...
            log_step("Calculate abundance statistics", "SUCCESS", 'taxonomy', 
                    avg_total_abundance=abundance_stats.avg_total_abundance,
                    avg_mean_abundance=abundance_stats.avg_mean_abundance,
                    avg_prevalence=abundance_stats.avg_prevalence,
                    total_records=abundance_stats.total_records)
